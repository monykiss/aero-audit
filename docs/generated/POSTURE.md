...
Governance index: **90%** (controls 85%, standards 100%, risk share low/medium 91%, studies runnable 95%). Basis: baseline (no session evidence).

## Domains

| Domain | Status | Rules | Controls implemented / partial / planned | Data sources |
|---|---|---|---|---|
| Air: surveillance integrity (ADS-B / Mode S) | active | 17 | 25 / 5 / 0 | adsb.lol /v2 (readsb JSON, NIC/NACp/SIL); OpenSky Network states/all; cross-feed corroboration |
| Air: operations, airports and ecosystem | active | 11 | 16 / 3 / 0 | adsb.lol / OpenSky; NOAA AWC METAR; FAA NAS status XML; bundled airport / operator / type tables; apron imagery |
...
| Space: launch and ascent | active | 8 | 11 / 6 / 0 | Launch Library 2 upcoming/previous (keyless, 15/h); telemetry CSV (t, speed, altitude); local footage; NASA library captions (.srt) |
| Space: orbital operations and conjunction | active | 16 | 9 / 5 / 0 | CelesTrak GP elements (keyless); CCSDS CDM 508.0-B files (KVN / XML); space-track.org cdm_public (free account); mission descriptions (JSON) |
...
| Air: UAS integration, detect-and-avoid, UTM | active | 5 | 4 / 4 / 0 | surveillance recordings (ADS-B, live or replayed); UTM operator/USS APIs (OpenAPI) |
