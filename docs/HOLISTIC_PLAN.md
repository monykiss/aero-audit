...
| 1. Space assets to training data | Render NASA 3D models from many viewpoints; pair with library imagery; fine-tune the detector; footage detections become real | **2026-09-15**: dataset pipeline (library + 3D previews, hashed, attributed, deterministic splits), scene classifier with card and registry gate, ultralytics fine-tune scaffold. **Later 2026-09-15**: numpy OBJ renderer for multi-view training images (`aero space render`). Open: the detector run itself (GPU) and OBJ conversions of the 3DS/LWO models |
