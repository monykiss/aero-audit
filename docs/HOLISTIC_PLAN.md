...
| 3. Orbital operations | TLE ingest (CelesTrak), SGP4 propagation, CDM parser, conjunction screening with Pc; debris-rule checklist | **First cut done 2026-09-10**: keyless elements, SGP4 screen, ORB-001..003, ST-06 runnable, C-09 partial (S05 residual critical → medium). Next: CDM parser with covariance and Pc; debris checklist (C-10) |
...
| 7b. Data governance registry | Metadata registry over recordings, assets, reports and study outputs modelled on CMR's collection/granule split, with cumulus-style reconciliation reports | `aero data` reconciles registry vs disk; every artefact searchable by time, space and provenance |
