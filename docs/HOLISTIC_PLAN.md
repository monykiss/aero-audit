...
| 1. Space assets to training data | Render NASA 3D models from many viewpoints; pair with library imagery; fine-tune the detector; footage detections become real | **2026-09-15**: dataset pipeline (library + 3D previews, hashed, attributed, deterministic splits), scene classifier with card and registry gate, ultralytics fine-tune scaffold. Open: rendered viewpoints and the detector run itself (GPU) |
...
| 6. Upstream | Catalogue and integrity tool offered to nasa/NASA-3D-Resources; library client published; branch becomes public | **2026-09-15**: standalone `scripts/nasa3d_catalog.py` (stdlib only) and the PR text in docs/UPSTREAM.md are ready; waiting on the licence confirmation (P-08) |
