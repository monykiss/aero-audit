...
<div class="row"><a class="btn small" href="#/findings?q=${icao}">All findings</a><button class="small" data-act="zoomTo" data-arg="${icao}">Zoom</button></div><h3>Findings (${d.findings.length})</h3>${fs}`;
  ...
  function row(f) { return `<div class="f s-${f.severity} ${f.injected ? 'inj' : ''}" data-act="focusNow" data-arg="${f.icao24 || ''}"><span class="sev ${f.severity}">${f.severity}</span><span class="t">${f.rule} · ${esc(f.callsign || f.icao24 || '-')}<small>${esc(f.title)}</small>${f.playbook ? `<small>→ ${esc(f.playbook)}</small>` : ''}</span><span class="r">${fmtTs(f.ts)}<br>risk ${f.risk}</span></div>`; }
  ...
  const P = {
    ...
    safety(s) { const em = s.kpis.emergencies; const met = s.metars.length ? `<h3>Weather (METAR)</h3><table>${s.metars.map(m => `<tr><td>${m.station}</td><td>wind ${m.wind} kt</td><td>vis ${m.visib}</td><td>${m.cover || ''}</td><td>${m.temp}°C</td></tr>`).join('')}</table>` : '<h3>Weather</h3><div class="note">no METAR (replay mode, or stations unavailable)</div>';
      return `<h3>Emergency / interference codes now (${em.length})</h3>${em.length ? em.map(a => `<div class="f s-critical" data-act="focusNow" data-arg="${a.icao24}"><span class="sev critical">${a.squawk}</span><span class="t">${esc(a.callsign || a.icao24)}<small>${num(a.alt)} ft · ${num(a.gs)} kt · ${a.src}</small></span><span class="r">trust ${a.trust}</span></div>`).join('') : '<div class="note ok">none: no 7500 / 7600 / 7700 in the current picture</div>'}
...
<h3>Low-trust aircraft (${lt.length})</h3>${lt.length ? `<table><tr><th>icao</th><th>trust</th><th>findings</th></tr>${lt.map(r => `<tr class="click" data-act="focusNow" data-arg="${r[0]}"><td>${r[0]}</td><td>${r[1].toFixed(2)}</td><td>${r[2]}</td></tr>`).join('')}</table>` : '<div class="note">none below 0.5</div>'}
...
<div class="mapwrap"><div id="map"></div><div id="ltip" class="ltip" hidden></div><div id="drawer"><span class="x" data-act="closeDrawer">✕</span><div id="drawerBody"></div></div>
...
<button data-k="flood" title="Sixty never-seen addresses at once">Flood</button><button data-k="collapse" title="60% of the picture vanishes for two polls">Collapse</button><button id="lclear" class="ghost small">Clear</button><button id="ltour" class="primary small" title="Scripted tour: all eight scenarios on a timeline, narrated in the banner">🎬 Tour</button></div>
<div class="tabs" id="ltabs"><div data-t="feed" class="on">Findings</div><div data-t="safety">Safety</div><div data-t="security">Security</div><div data-t="ops">Operations</div></div><div class="panel" id="lpanel"></div></aside></div>`;
      ...
      el.querySelector('#ldemo').addEventListener('click', async e => { const b = e.target.closest('button'); if (!b) return;
        const hdr = {...window.App.headers(), 'Content-Type': 'application/json'};
        if (b.id === 'lclear') { await fetch('/api/v1/clear', {method: 'POST', headers: hdr, body: '{}'}); return; }
        if (b.id === 'ltour') { window.App.tour(window.App.tourRunning && window.App.tourRunning() ? 'stop' : 'start'); return; }
        ...
        const r = await fetch('/api/v1/inject', {method: 'POST', headers: hdr, body: JSON.stringify(body)}); const j = await r.json();
