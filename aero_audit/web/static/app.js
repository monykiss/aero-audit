...
<div class="filters">${sel('laction', 'action', d.actions, q.action)}${sel('lactor', 'actor', ['user', 'system'], q.actor)}<input type="text" id="lq" placeholder="search details" value="${esc(q.q || '')}"><button class="primary" id="lgo">Apply</button><span class="grow"></span><a class="btn" href="${API}/audit.csv" download>CSV</a></div><div id="lgrid"></div>`;
    ...
    governance: { title: 'Governance', async render(el) {
      const p = await api('/governance'); const c = p.components; const f = p.freshness || {}; const q = store.query;
      const pct = v => v == null ? '—' : Math.round(v * 100) + '%';
      el.innerHTML = `<h1>Governance <span class="note">air + space · compliance · risk · study · governing · basis: ${esc(p.evidence_basis)}</span></h1>
<p class="lead">One view over every domain: which controls exist and what evidence backs them, where residual risk sits across air and space, which studies can run today, and whether the evidence on disk is fresh. The index moves only when those move.</p>
<div class="tiles"><div class="tile c-amber"><b>${pct(p.governance_index)}</b><span>governance index</span></div><div class="tile c-green"><b>${pct(c.controls_implementation)}</b><span>controls implemented</span></div><div class="tile c-blue"><b>${pct(c.standards_coverage)}</b><span>standards touched</span></div><div class="tile c-violet"><b>${pct(c.risk_share_low_or_medium)}</b><span>risks at low / medium residual</span></div><div class="tile c-teal"><b>${p.studies.runnable}/${p.studies.rows.length}</b><span>studies runnable</span></div><div class="tile ${f.audit_chain_ok ? 'c-green' : 'c-red'}"><b>${f.audit_chain_ok ? 'OK' : (f.audit_chain_ok === null ? 'none' : 'BROKEN')}</b><span>audit chain</span></div><div class="tile ${f.model_verified ? 'c-green' : 'c-grey'}"><b>${f.model_verified == null ? '—' : (f.model_verified ? 'verified' : 'NO')}</b><span>model vs registry</span></div></div>
<h2>Domains</h2><div id="gdom"></div>
<h2>Controls</h2><div class="filters">${sel('gpil', 'pillar', ['compliance', 'risk', 'study', 'governance'], q.pillar)}${sel('gst', 'status', ['implemented', 'partial', 'planned'], q.status)}<button class="primary" id="ggo">Apply</button><span class="grow"></span><span class="note">evidence types: rule · scenario · test · module · artefact · command · workflow · doc · study</span></div><div id="gctl"></div>
<h2>Unified risk register (residual)</h2><div id="grisk"></div>
<h2>Studies</h2><div id="gstud"></div>
<h2>Policies</h2><div id="gpol"></div>
${p.evidence.missing.length ? `<div class="card accent-amber" style="margin-top:12px"><b class="amb">Evidence claimed but not on disk:</b> ${p.evidence.missing.map(esc).join(' · ')}</div>` : ''}`;
      grid(el.querySelector('#gdom'), [{k: 'name', label: 'Domain', fmt: r => `<b>${esc(r.name)}</b><br><span class="note">${esc(r.description).slice(0, 140)}</span>`}, {k: 'status', label: 'Status', cls: r => r.status === 'active' ? 'pos' : (r.status === 'planned' ? 'amb' : '')}, {k: 'rules', label: 'Rules', num: true}, {k: 'controls', label: 'Impl / partial / planned', fmt: r => `${r.controls.implemented} / ${r.controls.partial} / ${r.controls.planned}`}, {k: 'data_sources', label: 'Data sources', fmt: r => esc(r.data_sources.join('; '))}, {k: 'upstream', label: 'Adopts from', fmt: r => esc(r.upstream.join(', '))}], p.domains, {sort: 'status', asc: true});
      const ctl = p.controls.filter(r => (!q.pillar || r.pillar === q.pillar) && (!q.status || r.status === q.status));
      grid(el.querySelector('#gctl'), [{k: 'id', label: 'Id'}, {k: 'pillar', label: 'Pillar'}, {k: 'title', label: 'Control', fmt: r => `<b>${esc(r.title)}</b><br><span class="note">${esc(r.objective)}</span>`}, {k: 'status', label: 'Status', cls: r => r.status === 'implemented' ? 'pos' : (r.status === 'planned' ? 'amb' : '')}, {k: 'domains', label: 'Domains', fmt: r => esc(r.domains.join(', '))}, {k: 'standards', label: 'Standards', fmt: r => esc(r.standards.join(', '))}, {k: 'evidence', label: 'Evidence', fmt: r => r.evidence.map(e => `<span class="${r.evidence_missing.includes(e) ? 'neg' : ''}">${esc(e)}</span>`).join('<br>') || '<span class="note">none yet</span>'}], ctl, {sort: 'id', asc: true});
      grid(el.querySelector('#grisk'), [{k: 'id', label: 'Id'}, {k: 'domain', label: 'Domain'}, {k: 'title', label: 'Risk'}, {k: 'L', label: 'L', num: true}, {k: 'I', label: 'I', num: true}, {k: 'score', label: 'Inherent', num: true, fmt: r => `<span class="rat-${r.rating}">${r.score} ${r.rating}</span>`}, {k: 'residual', label: 'Residual', num: true, fmt: r => `<span class="rat-${r.residual_rating}">${r.residual} ${r.residual_rating}</span>`}, {k: 'controls', label: 'Controls / evidence', fmt: r => esc((r.controls && r.controls.length ? r.controls : (r.evidence || [])).join(', '))}], p.risks.rows, {sort: 'residual'});
      const latest = p.studies.latest || {};
      grid(el.querySelector('#gstud'), [{k: 'id', label: 'Id'}, {k: 'title', label: 'Study', fmt: r => `<b>${esc(r.title)}</b><br><span class="note">${esc(r.question)}</span>`}, {k: 'domain', label: 'Domain'}, {k: 'status', label: 'Status', cls: r => r.status === 'runnable' ? 'pos' : 'amb'}, {k: 'inputs', label: 'Inputs', fmt: r => esc(r.inputs.join(', '))}, {k: 'upstream', label: 'Method from', fmt: r => esc(r.upstream.join(', '))}, {k: 'last', label: 'Last result', fmt: r => latest[r.id] ? esc(latest[r.id].file.split('/').pop()) : ''}], p.studies.rows, {sort: 'id', asc: true});
      grid(el.querySelector('#gpol'), [{k: 'id', label: 'Id'}, {k: 'title', label: 'Policy'}, {k: 'statement', label: 'Statement'}, {k: 'owner', label: 'Owner'}, {k: 'controls', label: 'Controls', fmt: r => esc(r.controls.join(', '))}], p.policies, {sort: 'id', asc: true});
...
${(() => { const st = integ.items.find(i => i.key === 'spacetrack'); return st && st.configured ? '' : `<div class="card accent-amber" style="margin:8px 0"><b class="amb">Space-Track is optional and not connected.</b> Nothing here depends on it: conjunctions come from CelesTrak elements and the SGP4 screen, history from cached snapshots, decay dates and identity from the CelesTrak SATCAT. If you do connect it (<code>aero accounts-setup spacetrack</code>), its user agreement forbids redistributing its data or analyses, and the tool keeps that material out of bundles.</div>`; })()}
<div class="row"><button class="primary" id="spconj">Screen conjunctions (CelesTrak stations)</button>${integ.items.find(i => i.key === 'spacetrack' && i.configured) ? '<button id="spst">Pull Space-Track summaries</button>' : ''}<button id="spmvb">Element history</button><button id="spsatb">Fetch SATCAT (decays, identity)</button><button id="spcdm">Process CDM inbox</button><button id="spswx">Fetch space weather</button><button id="splch">Fetch launch windows</button><select id="sprec">${recOpts}</select><button id="spjoin">Join launches to this recording</button><button id="spexp">Exposed flights (space weather)</button></div>
...
<h2>Airspace (FAA space-operations TFRs, keyless) ${s.tfr ? `<span class="note">${esc(s.tfr.file)} · fetched ${esc(s.tfr.fetched_at)}</span>` : '<span class="note">no product cached; fetch one</span>'}</h2><div class="tiles">${s.tfr ? `<div class="tile c-blue"><b>${s.tfr.features}</b><span>space-ops restrictions</span></div><div class="tile ${s.tfr.active_now ? 'c-amber' : 'c-green'}"><b>${s.tfr.active_now}</b><span>in effect now</span></div><div class="tile c-teal"><b>${s.tfr.with_geometry}</b><span>with geometry</span></div><div class="tile c-grey"><b>${s.tfr.listed_total}</b><span>TFRs of every type listed</span></div>` : ''}</div><div class="row"><button id="sptfr">Fetch TFRs (+ launch coverage)</button><button id="spren">Reentry corridors (cached elements)</button></div>
...
${sw && sw.donki ? `<div class="card">NASA DONKI cross-check (${esc(sw.donki.file)}${sw.donki.own_key ? ', your key' : ', DEMO_KEY'}, ${sw.donki.recent_notifications} notification(s) in ${sw.donki.window_h} h): ${Object.entries(sw.donki.effects).map(([k, v]) => `<b>${k}</b> ${esc(v.agreement)}${v.types.length ? ' (' + esc(v.types.join(', ')) + ')' : ''}`).join(' · ')}<br><span class="note">${esc(sw.donki.note)}</span></div>` : ''}
<h2>Launch windows ${lc ? `<span class="note">${esc(lc.file)} · ${esc(lc.mode)} · fetched ${esc(lc.fetched_at)}</span>` : ''}</h2><div id="splcht"></div>
<h2>Conjunction screen ${cj.report ? `<span class="note">${esc(cj.report)} · ${cj.pairs} pairs · ${ago(cj.mtime)}</span>` : '<span class="note">none yet</span>'}</h2><div id="spcj"></div>
<h2>Conjunction data messages ${cd.report ? `<span class="note">latest ${esc(cd.report)}</span>` : ''}</h2>${cd.assessment ? `<div class="card">${Object.entries(cd.assessment).filter(([k, v]) => typeof v !== 'object').slice(0, 10).map(([k, v]) => `<b>${esc(k)}</b> ${esc(v)}`).join(' · ')}</div>` : ''}<div id="spev"></div><div id="spcdf"></div>
<h2>Debris checklist ${db.report ? `<span class="note">${esc(db.report)}</span>` : '<span class="note">run aero space debris &lt;mission.json&gt;</span>'}</h2><div id="spdb"></div>
<h2>Catalogue (CelesTrak SATCAT, keyless) ${s.satcat ? `<span class="note">${esc(s.satcat.file)} · ${s.satcat.objects.toLocaleString()} objects · ${s.satcat.on_orbit.toLocaleString()} on orbit · ${s.satcat.age_h} h old</span>` : '<span class="note">not cached; fetch it</span>'}</h2><div id="spsat"></div>
<h2>Element history ${s.maneuvers.report ? `<span class="note">${esc(s.maneuvers.report)} · ${s.maneuvers.changes.length} manoeuvre-scale changes · ${s.maneuvers.decaying.length} decaying</span>` : '<span class="note">needs two or more cached element files</span>'}</h2><div id="spmv"></div>
<h2>Element sets</h2><div id="spel"></div>
<h2>Assets, dataset, classifier</h2><div class="card">NASA-3D files: <b>${as.nasa3d_files ?? '—'}</b> · media items: <b>${as.media_items}</b> · dataset items: <b>${as.dataset_items}</b> ${as.dataset_classes ? esc(JSON.stringify(as.dataset_classes)) : ''} · classifier: ${as.classifier ? `trained ${esc(as.classifier.trained_at)} · accuracy ${Math.round((as.classifier.accuracy || 0) * 100)}% · sha ${esc((as.classifier.sha256 || '').slice(0, 12))}` : 'none'}</div>
<h2>Integrations <span class="note">credentials only from the environment; values never shown</span></h2><div id="spint"></div>
<h2>Scheduled intake <span class="note">${esc(sched.env)}=job=seconds,… · ${sched.offline ? 'offline: network jobs skipped' : 'online'}</span></h2><div id="spsch"></div>
<h2>Reports</h2><div id="sprep"></div>`;
      if (sw) grid(el.querySelector('#spswxf'), [{k: 'rule_id', label: 'Rule'}, {k: 'severity', label: 'Sev', cls: r => sevc(r.severity)}, {k: 'title', label: 'Finding', fmt: r => esc(r.title)}], sw.findings_list, {sort: 'rule_id', asc: true});
      grid(el.querySelector('#splcht'), [{k: 'name', label: 'Launch', fmt: r => esc(r.name)}, {k: 'provider', label: 'Provider'}, {k: 'status', label: 'Status', cls: r => r.status === 'Go' ? 'pos' : ''}, {k: 'window_start', label: 'Window start'}, {k: 'window_end', label: 'Window end'}, {k: 'pad', label: 'Pad'}, {k: 'location', label: 'Location'}], lc ? lc.rows : [], {sort: 'window_start', asc: true});
      grid(el.querySelector('#spcj'), fcols, cj.findings, {sort: 'severity'});
      grid(el.querySelector('#spev'), [{k: 'pair', label: 'Pair'}, {k: 'tca', label: 'TCA'}, {k: 'messages', label: 'Msgs', num: true}, {k: 'pc_latest', label: 'Pc latest', fmt: r => r.pc_latest == null ? '—' : Number(r.pc_latest).toExponential(2)}, {k: 'trend', label: 'Trend', cls: r => r.trend === 'escalating' ? 'neg' : (r.trend === 'de-escalating' ? 'pos' : '')}, {k: 'miss_m', label: 'Miss m', num: true}], cd.events, {sort: 'pc_latest'});
      grid(el.querySelector('#spcdf'), fcols, cd.findings, {sort: 'severity'});
      grid(el.querySelector('#spdb'), fcols, db.findings, {sort: 'severity'});
      grid(el.querySelector('#spmv'), fcols, s.maneuvers.findings, {sort: 'severity'});
      grid(el.querySelector('#spsat'), [{k: 'norad', label: 'NORAD', num: true}, {k: 'name', label: 'Object', fmt: r => esc(r.name)}, {k: 'type', label: 'Type'}, {k: 'owner', label: 'Owner'}, {k: 'decay_date', label: 'Decayed'}], s.satcat ? s.satcat.recent_decays : [], {sort: 'decay_date'});
      grid(el.querySelector('#spel'), [{k: 'file', label: 'File'}, {k: 'sets', label: 'Sets', num: true}, {k: 'fetched_at', label: 'Fetched'}, {k: 'age_h', label: 'Age h', num: true, cls: r => r.age_h > 168 ? 'neg' : (r.age_h > 48 ? 'amb' : '')}], s.elements, {sort: 'age_h', asc: true});
      grid(el.querySelector('#spint'), [{k: 'name', label: 'Service'}, {k: 'configured', label: 'Configured', fmt: r => r.required ? (r.configured ? '<span class="pos">yes</span>' : '<span class="amb">no</span>') : '<span class="note">keyless</span>'}, {k: 'unlocks', label: 'Unlocks', fmt: r => esc(r.unlocks)}, {k: 'keyless', label: 'Without it', fmt: r => esc(r.keyless)}, {k: 'env', label: 'Env', fmt: r => esc(r.env.concat(r.optional_env).join(' '))}, {k: 'signup', label: 'Sign up', fmt: r => esc(r.signup)}], integ.items, {sort: 'name', asc: true});
      grid(el.querySelector('#spsch'), [{k: 'job', label: 'Job'}, {k: 'every_s', label: 'Every s', num: true}, {k: 'next_in_s', label: 'Next in s', num: true}, {k: 'runs', label: 'Runs', num: true}, {k: 'skipped_offline', label: 'Skipped', num: true}, {k: 'last_run', label: 'Last run', fmt: r => fmtTs(r.last_run)}], sched.entries, {sort: 'job', asc: true});
      const reps = []; for (const [k, rows] of Object.entries(s.reports)) for (const r of rows) reps.push({kind: k, ...r});
      grid(el.querySelector('#sprep'), [{k: 'kind', label: 'Kind'}, {k: 'name', label: 'Report', fmt: r => `<a href="${r.url}" target="_blank">${esc(r.name)}</a>`}, {k: 'findings', label: 'Findings', num: true}, {k: 'mtime', label: 'When', fmt: r => ago(r.mtime)}], reps, {sort: 'mtime'});
      el.querySelector('#spconj').onclick = () => App.runJob('conjunctions', {group: 'stations'}, 'space');
      el.querySelector('#spcdm').onclick = () => App.runJob('cdm_inbox', {}, 'space');
      el.querySelector('#spmvb').onclick = () => App.runJob('maneuvers', {}, 'space');
      el.querySelector('#spsatb').onclick = () => App.runJob('satcat', {refresh: true}, 'space');
      const spst = el.querySelector('#spst'); if (spst) spst.onclick = () => App.runJob('spacetrack_pull', {days: 7}, 'space');
      el.querySelector('#spswx').onclick = () => App.runJob('space_weather', {}, 'space');
      el.querySelector('#sptfr').onclick = () => App.runJob('tfr', {}, 'space');
      el.querySelector('#spren').onclick = () => App.runJob('reentry', {}, 'space');
...
<h2>Encounter model ${u.encounter_model.report ? `<span class="note">${esc(u.encounter_model.report)}</span>` : '<span class="note">fit from a recording, then simulate</span>'}</h2>${u.encounter_model.simulation ? `<div class="card">${(() => { const m = u.encounter_model.model, sm = u.encounter_model.simulation; return `fitted from <b>${m.n}</b> encounter pairs (${m.flight_hours} fh) · simulated <b>${sm.n}</b> · P(NMAC) unmitigated <b>${sm.p_nmac_unmitigated}</b> · with a ${sm.horizon_s} s alerting horizon <b>${sm.p_nmac_mitigated}</b> · model risk ratio <b>${sm.risk_ratio ?? '—'}</b>${sm.rates ? ` · NMAC per fh ${sm.rates.nmac_per_fh_unmitigated} → ${sm.rates.nmac_per_fh_mitigated}` : ''}<br><span class="note">${esc(sm.basis)}</span>`; })()}</div>` : ''}
<h2>UTM contract checks ${u.utm.report ? `<span class="note">${esc(u.utm.report)}</span>` : '<span class="note">aero uas utm-check &lt;openapi.json&gt; &lt;samples&gt;</span>'}</h2><div id="uutm"></div>
<h2>Reports</h2><div id="urep"></div>`;
      grid(el.querySelector('#upairs'), [{k: 'callsigns', label: 'Pair'}, {k: 'region', label: 'Region'}, {k: 'min_range_ft', label: 'Min range ft', num: true, fmt: r => Math.round(r.min_range_ft)}, {k: 'min_dz_ft', label: 'Min dz ft', num: true}, {k: 'min_hmd_ft', label: 'Min HMD ft', num: true, fmt: r => Math.round(r.min_hmd_ft)}, {k: 'max_alert', label: 'Alert', num: true}, {k: 'violation', label: 'Violation', fmt: r => r.violation ? '<span class="neg">yes</span>' : 'no'}, {k: 'nmac_proximate', label: 'NMAC-prox', fmt: r => r.nmac_proximate ? '<span class="neg">yes</span>' : ''}, {k: 'lead_time_s', label: 'Lead s', num: true}], u.wellclear.pairs, {sort: 'min_range_ft', asc: true});
      grid(el.querySelector('#ufind'), fcols, u.wellclear.findings.concat(u.risk.findings), {sort: 'severity'});
      const bands = dens ? Object.entries(dens.bands).map(([band, v]) => ({band, aircraft_hours: v.aircraft_hours, cells: v.cells, ...Object.fromEntries(['sparse', 'moderate', 'dense', 'very-dense'].map(c => [c, v.classes[c] || 0]))})) : [];
      grid(el.querySelector('#udens'), [{k: 'band', label: 'Band ft'}, {k: 'aircraft_hours', label: 'Aircraft-hours', num: true}, {k: 'cells', label: 'Cells', num: true}, {k: 'sparse', label: 'Sparse', num: true}, {k: 'moderate', label: 'Moderate', num: true}, {k: 'dense', label: 'Dense', num: true, cls: r => r.dense ? 'amb' : ''}, {k: 'very-dense', label: 'Very dense', num: true, cls: r => r['very-dense'] ? 'neg' : ''}], bands, {sort: 'band', asc: true});
      grid(el.querySelector('#uutm'), fcols, u.utm.findings, {sort: 'severity'});
      const reps = []; for (const [kk, rows] of Object.entries(u.reports)) for (const r of rows) reps.push({kind: kk, ...r});
      grid(el.querySelector('#urep'), [{k: 'kind', label: 'Kind'}, {k: 'name', label: 'Report', fmt: r => `<a href="${r.url}" target="_blank">${esc(r.name)}</a>`}, {k: 'findings', label: 'Findings', num: true}, {k: 'mtime', label: 'When', fmt: r => ago(r.mtime)}], reps, {sort: 'mtime'});
    ...
    observability: { title: 'Observability', async render(el) {
      ...
      el.innerHTML = `<h1>Observability <span class="note">metrics · structured logs · probes · v${esc(h.version)} · up ${Math.round(h.uptime_s / 60)} min</span></h1>
<p class="lead">Everything the process does is counted and timed: requests by route and status, ingest batches and engine time per batch, findings by rule, feed calls by host, jobs, alerts, audit entries. <code>/metrics</code> is Prometheus text, <code>/healthz</code> and <code>/readyz</code> are probes, every response carries <code>X-Request-Id</code>, and <code>logs/app.jsonl</code> is one JSON object per event.</p>
<div class="tiles"><div class="tile ${o.ready ? 'c-green' : 'c-red'}"><b>${o.ready ? 'READY' : 'NOT READY'}</b><span>readiness</span></div><div class="tile c-blue"><b>${n(k.requests_total)}</b><span>http requests</span></div><div class="tile ${k.denied_total ? 'c-amber' : 'c-grey'}"><b>${n(k.denied_total)}</b><span>refused by guard</span></div><div class="tile c-teal"><b>${n(k.http_p95_ms)} ms</b><span>state call p95</span></div><div class="tile c-violet"><b>${n(k.ingest_batches)}</b><span>batches ingested</span></div><div class="tile c-violet"><b>${n(k.ingest_states)}</b><span>state vectors</span></div><div class="tile c-teal"><b>${n(k.engine_p95_ms)} ms</b><span>engine p95 / batch</span></div><div class="tile c-amber"><b>${n(k.findings_total)}</b><span>findings raised</span></div><div class="tile ${k.source_errors ? 'c-red' : 'c-grey'}"><b>${n(k.source_errors)}</b><span>source errors</span></div><div class="tile c-grey"><b>${n(k.feed_requests)}</b><span>feed requests</span></div><div class="tile c-grey"><b>${n(k.jobs_total)}</b><span>jobs finished</span></div><div class="tile c-grey"><b>${n(k.audit_entries)}</b><span>audit entries</span></div></div>
<h2>Readiness checks</h2><div id="ochk"></div>
<h2>Counters</h2><div id="octr"></div>
<h2>Latencies</h2><div id="ohist"></div>
<h2>Recent log events <span class="note">${esc(o.log_file)}</span></h2><div class="filters">${sel('olvl', 'level', ['info', 'warning', 'error'], q.level)}<input type="text" id="oev" placeholder="event contains…" value="${esc(q.event || '')}"><button class="primary" id="ogo">Apply</button><span class="grow"></span><a class="btn" href="/metrics" target="_blank">/metrics</a><a class="btn" href="/readyz" target="_blank">/readyz</a></div><div id="olog"></div>`;
      grid(el.querySelector('#ochk'), [{k: 'name', label: 'Check'}, {k: 'ok', label: 'OK', fmt: r => r.ok ? '<span class="pos">yes</span>' : '<span class="neg">NO</span>'}, {k: 'detail', label: 'Detail', fmt: r => esc(r.detail)}],
        Object.entries(chk).map(([name, c]) => ({name, ok: c.ok, detail: Object.entries(c).filter(([kk]) => kk !== 'ok').map(([kk, v]) => `${kk}=${v}`).join(' · ')})), {sort: 'name', asc: true});
      const ctr = []; for (const [name, rows] of Object.entries(o.metrics.counters)) for (const r of rows) ctr.push({name, labels: Object.entries(r.labels).map(([a, b]) => `${a}=${b}`).join(', '), value: r.value});
      grid(el.querySelector('#octr'), [{k: 'name', label: 'Metric'}, {k: 'labels', label: 'Labels'}, {k: 'value', label: 'Value', num: true}], ctr, {sort: 'value'});
      const hs = []; for (const [name, rows] of Object.entries(o.metrics.histograms)) for (const r of rows) hs.push({name, labels: Object.entries(r.labels).map(([a, b]) => `${a}=${b}`).join(', '), count: r.count, p50: r.p50 == null ? null : Math.round(r.p50 * 1000), p95: r.p95 == null ? null : Math.round(r.p95 * 1000), p99: r.p99 == null ? null : Math.round(r.p99 * 1000)});
      grid(el.querySelector('#ohist'), [{k: 'name', label: 'Histogram'}, {k: 'labels', label: 'Labels'}, {k: 'count', label: 'Count', num: true}, {k: 'p50', label: 'p50 ms', num: true}, {k: 'p95', label: 'p95 ms', num: true}, {k: 'p99', label: 'p99 ms', num: true}], hs, {sort: 'count'});
      grid(el.querySelector('#olog'), [{k: 'ts', label: 'Time (UTC)', fmt: r => fmtTs(r.ts)}, {k: 'level', label: 'Level', cls: r => r.level === 'error' ? 'neg' : (r.level === 'warning' ? 'amb' : '')}, {k: 'event', label: 'Event'}, {k: 'request_id', label: 'Request', fmt: r => esc((r.request_id || '').slice(0, 12))}, {k: 'fields', label: 'Fields', fmt: r => esc(Object.entries(r).filter(([kk]) => !['ts', 'level', 'event', 'logger', 'request_id', 'traceparent'].includes(kk)).map(([kk, v]) => `${kk}=${typeof v === 'object' ? JSON.stringify(v) : v}`).join(' · ')).slice(0, 220)}], lg.items, {sort: 'ts'});
      el.querySelector('#ogo').onclick = () => location.hash = '#/observability?' + qs({level: el.querySelector('#olvl').value, event: el.querySelector('#oev').value});
    }, tick() { if (store.el && store.page === 'observability') { /* refreshed on navigation; metrics scrape is cheap but the page is dense */ } } },
...
<h3>Flights near ${a.iata} (${d.flights.length})</h3><div class="tablewrap" style="max-height:38vh"><table><tr><th>callsign</th><th>operator</th><th>type</th><th>phase</th><th>alt</th><th>gs</th><th>nm</th></tr>${d.flights.slice(0, 80).map(f => `<tr class="click" data-act="hash" data-arg="#/flights?icao=${f.icao24}"><td>${esc(f.callsign || f.icao24)}</td><td>${esc(f.operator_code || '')}</td><td>${esc(f.type || '')}</td><td>${phaseCell(f.phase)}</td><td class="num">${n0(f.alt)}</td><td class="num">${n0(f.gs)}</td><td class="num">${n0(f.airport_nm)}</td></tr>`).join('')}</table></div>`;
  ...
  const MNEMONICS = {HOME: '#/', LIVE: '#/live', FLT: '#/flights', AIRP: '#/airports', OPS: '#/operators', FIND: '#/findings', RISK: '#/risk', RPT: '#/reports', DATA: '#/data', LOG: '#/auditlog', GOV: '#/governance', SPACE: '#/space', UAS: '#/uas', OBS: '#/observability', SET: '#/settings', HELP: '#/help'};
