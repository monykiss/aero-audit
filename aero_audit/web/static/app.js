...
(function () {
  ...
  const findingRow = f => `<tr class="click s-${f.severity}" data-id="${f.id}"><td>${sevPill(f.severity)}</td><td>${f.rule}</td><td>${esc(f.callsign || '')}<br><span class="note">${f.icao24 || ''}${f.injected ? ' · <span style="color:#ff5cf0">injected</span>' : ''}</span></td><td>${fmtTs(f.ts)}</td><td>${f.risk}</td><td>${f.occurrences}</td><td>${esc(f.title)}</td></tr>`;
