...
def test_publish_checks_on_this_tree_leave_only_the_licence_item_open():
    ...
    assert set(rows) == {f"PUB-{i:02d}" for i in range(1, 11)}
    ...
    assert s["blocking_on_user"] and not s["ready"] and s["passed"] == 9
...
def test_secret_and_private_detection_on_a_scratch_repo(tmp_path, monkeypatch):
    ...
    (tmp_path / "notes.md").write_text("token ghp_ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789ab here\n")
